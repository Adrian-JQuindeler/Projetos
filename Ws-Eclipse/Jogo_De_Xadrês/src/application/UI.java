...
public class UI {
	...
	public static ChessPosition readChessPosition(Scanner sc) {
		try {
			...
			throw new InputMismatchException("Erro, valores permitidos: A1 - H8");
		}
	}

	private static void printCapturedPiece(List<ChessPiece> captured) {
		List<ChessPiece> BRANCO = captured.stream().filter(x -> x.getColor() == Color.BRANCO)
				.collect(Collectors.toList());
		List<ChessPiece> PRETO = captured.stream().filter(x -> x.getColor() == Color.PRETO)
				.collect(Collectors.toList());
